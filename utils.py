...
# from evolving_graph.scripts import read_script, read_script_from_string, read_script_from_list_string, \
...
def string_to_boolean(formula: str, curr_state: str) -> bool:
    ...
    formula_list = formula.replace('&', 'and').replace('|', 'or').replace('!', 'not ').replace('(', '( ').replace(')',
                                                                                                                  ' )').split()
...
def prop_level_traj(pose_list, graph, obj_ids, room_ids, mappings, radius=0.5):
    ...
    init_state.update(
        {room: hardcoded_truth_value_vh(pose_list[0], room, graph, radius, room=True) for room in room_ids})
...
def reprompt(translate_engine, valid_action2states, invalid_action, invalid_state_list, constraints, mappings,
             prompt_fpath="prompts/dialogue/explain_zeroshot_v2.txt"):
...
class manipulation_dict():
    ...
    def __init__(self, objs=None):
        self.action2pred = {'switchon': 'is_switchedon', 'open': 'is_open', 'grab': 'is_grabbed', 'touch': 'is_touched',
                            'putin': 'is_in', 'puton': 'is_in'}
...
def state_change_by_step_manipulation(comm, program, input_ltl, obj2id, room2id, obj_mappings, pred_mappings,
                                      init_position, init_room, env_num=0, stopped=False):
    """
...
def state_change_by_step_spot(program, input_ltl, nx_graph, obj_mapping, pred_mapping, obj2id, room2id, id2loc,
                              stopped=False):
    ...
    for i, line in enumerate(program):
        ...
        if any(x in line for x in NAV_ACTIONS):  # nav action
            ...
            try:
                prop_traj = check_nav_state_spot(goal_node, nx_graph, curr_node, obj_ids, room_ids, id2loc,
                                                 nav_mappings, radius=0.5)
            except:
                breakpoint()
            # append the same manip_states to nav_state for each element
            state_list_i = [concat_props(traj_i | manip_state) for traj_i in prop_traj]
...
def check_nav_state_spot(goal_node, nx_graph, curr_node, obj_ids, room_ids, id2loc, mappings, radius=2.0):
    ...
    init_state.update(
        {room: hardcoded_truth_value_spot(curr_loc, room_id, id2loc, radius, room=True) for room_id in room_ids})
    init_state_mapped = {mappings[k]: v for k, v in init_state.items()}
    ...
    for pose in pose_list[1:]:
        ...
        state.update(
            {room: hardcoded_truth_value_spot(pose, room, id2loc, radius=radius, room=True) for room in room_ids})
        new_state = [b for b in state.values()]
        if not state_buffer == new_state:
            prop_traj.append({mappings[k]: v for k, v in state.items()})
            state_buffer = new_state
